...
namespace MondialXboost.Web.Services
{
    ...
    {
        public const string StartMarker = "<!-- mondialxboost:snapshots:start -->";
        public const string EndMarker = "<!-- mondialxboost:snapshots:end -->";
        ...
        public async Task ExportAsync(CancellationToken ct = default)
        {
            ...
            var existing = File.Exists(readmePath) ? await File.ReadAllTextAsync(readmePath, ct) : "# MondialXboost";
        ...
        private string RepositoryRoot()
        {
            ...
            {
                if (File.Exists(Path.Combine(current.FullName, "MondialXboost.sln")))
        ...
        private static string TeamCell(string teamId, string teamName)
        {
            ...
            return string.IsNullOrWhiteSpace(flag)
                ? escaped
                : $"<img src=\"MondialXboost.Web/wwwroot/flags/4x3/{flag}.svg\" width=\"18\" alt=\"\"> {escaped}";
        }
