...
public abstract class TestFixtures
{
    ...
    protected static IOptions<MondialXboostConfig> AvailabilityOptions(string[] sources) =>
        Options.Create(new MondialXboostConfig
        {
    ...
    protected static async Task<MondialXboostDbContext> ImportedDb()
    {
        var db = await NewDb();
    ...
    protected static async Task<MondialXboostDbContext> NewDb()
    {
        var connection = new SqliteConnection("Data Source=:memory:");
        await connection.OpenAsync();
        var options = new DbContextOptionsBuilder<MondialXboostDbContext>().UseSqlite(connection).Options;
        var db = new MondialXboostDbContext(options);
    ...
    protected static string WebProjectRoot()
    {
        ...
        {
            var candidate = Path.Combine(current.FullName, "MondialXboost.Web");
            if (File.Exists(Path.Combine(candidate, "Data", MondialXboostDataFiles.GroupsCsv)))
                return candidate;

            current = current.Parent;
        }

        return Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "MondialXboost.Web"));
    }

    protected static string NewTempRoot()
    {
        var root = Path.Combine(Path.GetTempPath(), "MondialXboostTests", Guid.NewGuid().ToString("N"));
    ...
    {
        public string ApplicationName { get; set; } = "MondialXboost.Web.Tests";
