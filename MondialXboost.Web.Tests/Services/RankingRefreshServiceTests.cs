...
public class RankingRefreshServiceTests : TestFixtures
{
    ...
    public async Task RankingRefresh_WalksBackToLatestAvailableEloDateAndWritesParseableCsvs()
    {
        ...
        try
        {
            var options = Options.Create(new MondialXboostConfig
            {
            ...
            Assert.Equal(2, CsvParsingHelper.ReadCsv<FifaCsvRow>(Path.Combine(root, "Data", MondialXboostDataFiles.FifaRankingsCsv)).Count);
            Assert.Equal(2, CsvParsingHelper.ReadCsv<EloCsvRow>(Path.Combine(root, "Data", MondialXboostDataFiles.EloCsv)).Count);
        }
        finally
        {
    ...
    public async Task RankingRefresh_DoesNotOverwriteExistingCsvsWhenSourcesCannotParse()
    {
        ...
        try
        {
            ...
            var fifaPath = Path.Combine(data, MondialXboostDataFiles.FifaRankingsCsv);
            var eloPath = Path.Combine(data, MondialXboostDataFiles.EloCsv);
